...
async def show_products(chat_id, page=1, message_id=None, loading=False):
    """Render the shared product stock list. When message_id is supplied, edit the same Telegram message."""
    ...
    if ready:
        for idx, (product, stock) in enumerate(ready, start=1):
            ...
            price_text = fmt_amount(pricing["unit_price"], "IDR")
            lines.append(
                f"<b>{idx}.</b> {escape(str(product.get('name') or 'Product'))} "
                f"— <b>{price_text}</b> · Stok: <b>{stock_text}</b>"
            )
    else:
    ...
    if message_id is not None:
        return await edit2(chat_id, message_id, text, kb=markup)
    return await send2(chat_id, text, kb=markup)
...
async def show_product(chat_id, pid=None, message_id=None, product_index=None):
    ...
    if not product:
        ...
        if message_id is not None:
            return await edit2(chat_id, message_id, text, kb=back_keyboard())
        return await send2(chat_id, text, kb=back_keyboard())
    ...
    if message_id is not None:
        return await edit2(chat_id, message_id, text, kb=kb)
    return await send2(chat_id, text, kb=kb)
...
async def handle_callback2(cb):
    ...
    if data.startswith("b2:productnum:"):
        await show_product(
            ...
            product_index=int(data.split(":", 2)[2]),
        )
        return
...
async def handle_message2(message):    if "from" not in message or message["from"].get("is_bot"):
        ...
        for percent in steps + [100]:
            if loading_id:
                await edit2(
                    chat_id,
                    loading_id,
                    f"⏳ <b>Memuat data...</b>\n\n{await _progress_bar(percent)} <b>{percent}%</b>",
                )
    ...
    if text in ("📦 STOCK", "🏷️ List Produk", "🛒 List Produk", "/products", "1"):
        await set_b2_state(user["telegram_id"])
        await show_products(chat_id, 1)
        return
